...
**Issue 21 (Post-MVP Stability Refactor)**

Status: COMPLETE

Task: Resolve strict layout recursions and deadlocks in `SettingsView` by rewriting state synchronization. Initially `SettingsViewModel` used bidirectional binding logic via explicit `didSet` `isSyncing` loops which would crash or enter infinite layouts. Decouple by converting `AppConfig` struct serialization strictly onto unidirectional `@AppStorage` mechanisms and straight `@Published` variables. Fix LaunchAtLogin `SMAppService` toggles throwing synchronous state reverts.

Acceptance criteria:

- [x] Settings UI no longer locks up or crashes when toggling options repeatedly
- [x] Removed `isSyncing` custom logic in favor of pure unidirectional flow

**Issue 22 (Post-MVP Latency & Architecture Refactor)**

Status: COMPLETE

Task: Re-architect `AppCoordinator` to eliminate the ~500ms synchronous delay when hotkeys initialize `AVAudioEngine.start()`. Extract CoreAudio dependencies into a dedicated `AudioService` class. Extract CGEvent Accessibility triggers into a `ShortcutService` class. Shift audio startup logic to non-blocking async `Task.detached` threads.
